...
<body>
    ...
    <div class="containerUtenti">
        ...
        <c:if test="${utenti.size() == 1}">
            <button onclick="window.location.href='/farmacia/utenti';"
                class="btn btn-lg btn-primary btn-block torna" type="submit">Torna alla lista
                completa</button>
                                    ...
                                    <c:choose>
                                        <c:when test="${utente.bloccato == true}">
                                            <form action="/farmacia/utenti" method="post">
                                                <input type="hidden" value="${utente.email}" name="email" />
